...
(function(window){
	...
	u.bindClick = function(ctx){
	   ...
	   $("#btn_Search").click(function() {//查询
           var attrVal = $(".dropdown-text").attr("data-value");
		   var name = $("#searchName").val();
		   if(attrVal == '0'){
			   //用户名查询
			   window.location.href = ctx+"/back/users/userList?username="+name;
		   }else{
			   //手机号码查询
			   window.location.href = ctx+"/back/users/userList?mobile="+name;
		   }
       });
	   $.fn.tableSelected();//表单选中样式
		...
		$("#ul-edit").click(function(){
			...
			if(isSelected){
				...
				var idss = $(that).val();
				window.location.href = ctx+"/back/users/editUser/"+idss;
			}else{
	   ...
	   $("#ul-delete").click(function(){
		   ...
		   if(isSelected){
               ...
               parent.delConfig(obj,ctx,"/back/users/userDel");
		   }else{
			   layer.msg('您没有选中如何数据请选择后操作', {time:2000, icon:0});	
		   }
	   });
