        ...
        <div class="basicInfo ui-box">
            ...
            <div class="filedList">
                ...
                <input type="text" class="form-control inputXt" placeholder="用户名" name="username"
                       auto_color_flag="true" datatype="*" nullmsg="请输入用户名!" value="${user.username}"/>
            </div>
            <div class="filedList">
                <label>真实姓名</label>
                <input type="text" class="form-control inputXt" placeholder="真实姓名" name="realName"
                       auto_color_flag="true" datatype="*" nullmsg="请输入真实姓名!" value="${user.realName}"/>
            </div>
            <div class="filedList">
                <label>密码</label>
                <input type="password" class="form-control inputXt" placeholder="用户密码" name="password"
                       auto_color_flag="true" datatype="*6-100" nullmsg="请输入用户密码!" errormsg="密码至少6个字符,最多18个字符！" value="${user.password}"/>
            </div>
            <div class="filedList">
                <label>确认密码</label>
                <input type="password" class="form-control inputXt" placeholder="确认密码" name="rePassword"
                       auto_color_flag="true" datatype="*" recheck="password" nullmsg="请再次输入密码" value="${user.password}"/>
            </div>
            <div class="filedList">
                <label>所属角色</label>
            ...
            <div class="filedList">
                <label>身份证号码</label>
                <input type="text" class="form-control inputXt" placeholder="身份证号码" name="identityCardNum" value="${user.identityCardNum}"/>
            </div>
            <div class="filedList">
                <label>邮箱</label>
                <input type="text" class="form-control inputXt" placeholder="邮箱" name="email" value="${user.email}"/>
            </div>
            <div class="filedList">
                <label>是否可用</label>
...
<script type="text/javascript">
    $(function(){
        ...
        $scroll.initScroll();
        var roleIds = '${user.roleIdsStr}';
        $("input[type='checkbox']").each(function(){
            var val = $(this).val();
            console.log(val + "=="+roleIds);
            console.log(roleIds.indexOf(val));
            if(roleIds.indexOf(val) >= 0){
                $(this).prop("checked",true);
            }
        });
    });

    var isShow = ${user.locked};
