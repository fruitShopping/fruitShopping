...
<div class="table-main">
    ...
    <div class="response-head titlePanel">
        <div class="toolbar">
            <div class="btn-group">
                <a id="ul-add" class="btn btn-info" href="${ctx}/back/category/add">
    ...
    <div class="table-body-info">
        ...
        <table class="table table-responsive table-condensed listTable">
            ...
            <tbody>
            <c:forEach items="${cateList.dates}" var="category">
                <tr>
                    <td>
                        <input type="checkbox" name="checkbox" value="${category.id}"/>
                    </td>
                    <td>${category.categoryName}</td>
                    <td>${category.breedName}</td>
                    <td>
                        <c:choose>
                            <c:when test="${category.isShelves == 0}">
                                已上架
                            </c:when>
                            <c:otherwise>
                                未上架
    ...
    <!--分页-->
    <c:choose>
        <c:when test="${cateList.pageTotal > 1}">
...
<script type="text/javascript">
    $(function() {
        $categoryControl.initCategory('${ctx}');
        $checkBoxControl.initCheckBox();
        $pageControl.initPage('${cateList.pageNum}', '${url}', '${cateList.pageTotal}');
        $scroll.initScroll();
    });
